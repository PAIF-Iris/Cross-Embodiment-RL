...
path = "/Users/paif_iris/Desktop/metaworld/parquet/episode_000042.parquet"
...
meta_obj_start = np.array([0.0, 0.6, 0.02])
meta_obj_start_adjusted = np.array([0.0, 0.6, 0.02])
...
def teleport_and_step(env, target_pos, gripper, obj_pos, n_steps=10):
    for _ in range(n_steps):
        # 1. Set mocap target
        env.unwrapped.data.mocap_pos[0][:] = target_pos

        # 2. Zero velocities so no momentum fights the target
        env.unwrapped.data.qvel[:] = 0.0

        # 3. Run mujoco forward to recompute positions instantly
        mujoco.mj_forward(env.unwrapped.model, env.unwrapped.data)

        # 4. Now do one proper env.step to advance time + get obs
...
print("obs[0:3] after reset:", obs[0:4])
...
for i in range(len(df)-1):
    ...
    gripper_raw = df['action'].iloc[i+1][6]
    gripper = 1.0 if gripper_raw == -1 else -1.0

    target_pos = target_pos + delta
    print(f"Step {i}: target={np.round(target_pos, 4)}, gripper={gripper}")

    obs, reward, terminated, truncated, info = teleport_and_step(
        env, target_pos=target_pos, gripper=gripper, obj_pos=obj_pos
    )
    print(f"  -> actual EE pos: {np.round(obs[0:3], 4)}")

    if terminated:
        print(f"Task solved at step {i}!")
        break
    time.sleep(0.1)
